...
def pay_order(order):
    ...
    try:
        ...
        print(f"✅ 订单支付成功！订单号：{order['order_id']}")
        return True
    except Exception as e:
        print(f"❌ 订单支付失败：{str(e)}")
        return False

# 4. 定义订单查询函数
def query_order(order_id):
    """模拟查询订单"""
    # 测试场景下，直接返回模拟的订单信息（实际项目会查数据库/接口）
    mock_order = {
        "order_id": order_id,
        "status": "已支付",
        "total_amount": 2999.99
    }
    print(f"✅ 订单查询成功！订单信息：{mock_order}")
    return mock_order
